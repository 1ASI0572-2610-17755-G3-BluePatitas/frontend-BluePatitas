...
@Component({
  ...
  `,
  styles: [`
    button { width: 100%; min-height: 118px; display: grid; grid-template-columns: 82px minmax(0, 1fr) auto; grid-template-rows: 1fr auto; gap: 8px 14px; align-items: start; padding: 10px; border: 1px solid var(--bp-border); border-radius: 8px; background: white; text-align: left; cursor: pointer; box-shadow: 0 4px 14px rgba(11,31,47,.04); }
    ...
    strong { display: block; font-size: 17px; min-width: 0; overflow-wrap: anywhere; }
    small { display: block; color: var(--bp-slate-gray); min-width: 0; overflow-wrap: anywhere; font-size: 12px; }
    span { display: block; color: #334155; font-size: 12px; margin-top: 8px; }
    bp-status-chip { justify-self: end; }
    ...
    @media (max-width: 720px) {
      ...
      bp-status-chip { grid-column: 2; grid-row: 2; justify-self: start; }
    }
  `],
})
