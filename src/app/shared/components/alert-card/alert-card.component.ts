...
@Component({
  ...
  template: `
    <article [class.critical]="alert.severity === 'Critical'">
      <span>{{ alert.severity === 'Critical' ? 'priority_high' : 'notifications' }}</span>
      <div>
  ...
  styles: [`
    ...
    span { display: grid; place-items: center; height: 42px; border-radius: 999px; background: rgba(0, 91, 176, .1); color: var(--bp-action-blue); font-family: 'Material Icons'; }
    .critical span { background: rgba(217, 48, 37, .12); color: var(--bp-critical); }
