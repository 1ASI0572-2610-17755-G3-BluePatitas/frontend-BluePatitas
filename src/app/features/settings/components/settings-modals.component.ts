  ...
  template: `
    <bp-modal [open]="open" [title]="'settings.editShelter' | translate" size="compact" (closed)="closed.emit()">
      <div class="modal-body form-grid">
        ...
        <bp-form-field [label]="'settings.shelterName' | translate" placeholder="Blue Patitas" [(value)]="name" />
        <bp-form-field [label]="'settings.mainAddress' | translate" placeholder="Av. Las Flores 123" [(value)]="address" />
        <bp-form-field [label]="'auth.phone' | translate" placeholder="+51 987 654 321" [(value)]="phone" />
        <bp-form-field [label]="'auth.email' | translate" placeholder="contacto@bluepatitas.com" [(value)]="email" />
        ...
        <div class="modal-actions">
          ...
          <bp-button (clicked)="onSave()">{{ 'settings.saveChanges' | translate }}</bp-button>
        </div>
      </div>
    </bp-modal>
  `,
...
export class EditShelterDataModalComponent implements OnChanges {
  @Input() open = false;
  @Input() shelter?: Shelter;
  @Output() closed = new EventEmitter<void>();
  @Output() saved = new EventEmitter<Shelter>();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['open']?.currentValue || changes['shelter']) {
  ...
  onSave(): void {
    const updated: Shelter = {
      id: this.shelter?.id || 'shelter-wuf',
      name: this.name,
      address: this.address,
      phone: this.phone,
      email: this.email,
      city: this.shelter?.city || 'Lima',
      administrator: this.shelter?.administrator || 'Administrator',
    };
