...
import { GetShelterSettingsUseCase, UpdateShelterSettingsUseCase } from '../../core/application/use-cases/bluepatitas.use-cases';
import { Shelter } from '../../core/domain/models/bluepatitas.models';
      ...
      @if (stepNumber() === 3) {
        <section class="setup-card confirmation-card">
          ...
          <article class="shelter-summary">
            ...
            <dl>
              <div><span class="summary-icon building"></span><dt>{{ 'onboarding.shelterName' | translate }}</dt><dd>{{ summaryName }}</dd></div>
              <div><span class="summary-icon pin"></span><dt>{{ 'onboarding.address' | translate }}</dt><dd>{{ summaryAddress }}</dd></div>
              <div><span class="summary-icon map"></span><dt>{{ 'onboarding.districtCity' | translate }}</dt><dd>{{ summaryDistrictCity }}</dd></div>
            </dl>
          </article>
        ...
        <section class="setup-card form-card">
          ...
          <div class="form-grid">
            @if (stepNumber() === 1) {
              <bp-form-field [label]="'onboarding.shelterNameRequired' | translate" [(value)]="name" />
              <bp-form-field label="RUC" [(value)]="ruc" />
              <bp-form-field [label]="'onboarding.institutionalEmailRequired' | translate" [(value)]="email" />
              <bp-form-field [label]="'onboarding.contactPhoneRequired' | translate" [(value)]="phone" />
            } @else {
              <bp-form-field [label]="'onboarding.addressRequired' | translate" [placeholder]="'onboarding.addressPlaceholder' | translate" [(value)]="address" />
              <bp-form-field [label]="'onboarding.referenceOptional' | translate" [placeholder]="'onboarding.referencePlaceholder' | translate" [(value)]="reference" />
              <bp-form-field [label]="'onboarding.districtRequired' | translate" [placeholder]="'onboarding.districtPlaceholder' | translate" [(value)]="district" />
              <bp-form-field [label]="'onboarding.cityRequired' | translate" [placeholder]="'onboarding.cityPlaceholder' | translate" [(value)]="city" />
            }
          </div>
          ...
          <footer class="form-footer">
            ...
            <bp-button (clicked)="onNext()">{{ stepNumber() === 1 ? ('onboarding.next' | translate) : ('onboarding.createShelter' | translate) }}</bp-button>
          </footer>
        </section>
      }
...
export class OnboardingPage implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    const saved = localStorage.getItem('onboarding_shelter');
    if (saved) {
    ...
    if (this.stepNumber() === 3) {
      ...
      this.summaryName = shelter.name || 'Refugio Esperanza';
      this.summaryAddress = shelter.address || 'Sede Norte';
      this.summaryDistrictCity = `${shelter.city || 'Lima'}`;
    }
  }
  ...
  async onNext(): Promise<void> {
    if (this.stepNumber() === 1) {
      ...
      localStorage.setItem('onboarding_shelter', JSON.stringify(data));
      void this.router.navigate(['/onboarding/refuge/location']);
      ...
      const updatedShelter: Shelter = {
        ...
        address: this.address,
        phone: parsed.phone || this.phone,
        email: parsed.email || this.email,
        city: this.city,
        administrator: 'Administrator'
      };

      await this.updateShelter.execute(updatedShelter);
      localStorage.removeItem('onboarding_shelter');
      void this.router.navigate(['/onboarding/refuge/confirmation']);
    }
  }
